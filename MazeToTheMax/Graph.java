...
public class Graph {
    ...
    public ArrayList<Edge> getOptions(int vertex){
        ArrayList<Edge> options = new ArrayList<Edge>();
        for (int i = 0; i < edges.size(); i++) {
            if(edges.get(i).getStart() == vertex && !edges.get(i).getBlock()){
    ...
    public Graph getValidEdges(){
        Graph valid = this;
        for (int i = 0; i < edges.size(); i++) {
            if(this.getEdges().get(i).getBlock()){
                valid.getEdges().remove(edges.get(i));
            }
        }
        return valid;
    }
